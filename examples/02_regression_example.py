...
from reptree.utils import train_test_split, export_text, plot_tree_stats
from reptree.metrics import mean_squared_error, r2_score, mean_absolute_error
...
def main():
    ...
    print(f"   {'Test MSE':<25} {test_mse_no_prune:<15.4f} {test_mse_prune:<15.4f} {mse_change:+.4f}")
    print(f"   {'Test R²':<25} {test_r2_no_prune:<15.4f} {test_r2_prune:<15.4f} {r2_change:+.4f}")
    print(f"   {'Tree Depth':<25} {reg_no_prune.get_depth():<15} {reg_with_prune.get_depth():<15} {reg_with_prune.get_depth() - reg_no_prune.get_depth():+}")
    print(f"   {'Number of Nodes':<25} {reg_no_prune.tree_.count_nodes():<15} {reg_with_prune.tree_.count_nodes():<15} {-node_reduction:+}")
    ...
    tree_text = export_text(reg_with_prune.tree_, feature_names=feature_names, max_depth=3, decimals=3)
    print(tree_text)
