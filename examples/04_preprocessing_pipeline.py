...
def create_messy_dataset(n_samples=500, random_state=42):
    ...
    employment_score = np.array([3 if e == 'Full-time' else 2 if e == 'Self-employed' 
                                  else 1 if e == 'Part-time' else 0 for e in employment])
    ...
    for i, name in enumerate(feature_names):
        ...
        if non_missing:
            ...
            try:
                ...
                if outliers > 0:
                    print(f"  Potential outliers: {outliers} ({outliers/len(numeric_vals)*100:.1f}%)")
            except (ValueError, TypeError):
                print(f"  Type: Categorical")
                unique_vals = set(non_missing)
                print(f"  Unique values: {len(unique_vals)}")
                print(f"  Categories: {list(unique_vals)[:5]}{'...' if len(unique_vals) > 5 else ''}")
