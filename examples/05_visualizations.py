...
def plot_tree_structure(model: REPTreeClassifier, feature_names: Optional[list], fname: str) -> None:
    out = ensure_output_dir() / fname
    tree = model.tree_
    if tree is None:
        return
...
def plot_feature_importance(model: REPTreeClassifier, feature_names: Optional[list], fname: str) -> None:
    out = ensure_output_dir() / fname
...
def plot_decision_boundary_2d(model: REPTreeClassifier, X: np.ndarray, y: np.ndarray, fname: str) -> None:
    out = ensure_output_dir() / fname
...
def plot_confusion_and_report(model: REPTreeClassifier, X: np.ndarray, y: np.ndarray, fname: str, target_names: Optional[list] = None) -> None:
    out = ensure_output_dir() / fname
...
def plot_pruning_impurity_deltas(X: np.ndarray, y: np.ndarray, fname_prefix: str) -> None:
    ...
    def draw_nodes(node):
        ...
        ax.scatter([x], [y], s=300, color=color, zorder=3)
        ax.text(x, y, f"id#{node.node_id}\nΔ={d:.3f}", ha="center", va="center", color="white", fontsize=8)
        if node.left:
            draw_nodes(node.left)
        if node.right:
            draw_nodes(node.right)
...
def run_iris_visualizations():
    ...
    # Dataset
    plot_distributions(df, target=target_col, title="Iris Feature Distributions", fname="iris_distributions.png")
    plot_pairplot(df, target=target_col, fname="iris_pairplot.png")
    plot_correlations(df, target=target_col, title="Iris Correlations", fname="iris_correlations.png")
    # Model
    Xtr, Xte, ytr, yte = train_test_split(X, y, test_size=0.3, random_state=42)
    ...
    plot_confusion_and_report(clf, Xte, yte_codes, fname="iris_confusion.png", target_names=list(labels))
...
def run_bike_regression_visualizations():
    ...
    plot_correlations(df, target=None, title="Bike Hour Correlations", fname="bike_correlations.png")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", choices=["iris", "bike", "all"], default="all", help="Select dataset")
    parser.add_argument(
        "--plots",
        nargs="*",
        choices=[
            "dataset", "model", "boundaries", "training", "errors", "pruning", "pruning-advanced", "regression"
        ],
