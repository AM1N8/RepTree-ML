...
class DataPreprocessor:
    ...
    def _transform_missing_values(self, feat_idx: int, feature_col: np.ndarray) -> np.ndarray:
        ...
        if self.handle_missing == 'drop':
            ...
            warnings.warn("'drop' strategy for missing values should be applied to entire rows", UserWarning)
            return feature_col
    ...
    def _transform_categorical(self, feat_idx: int, feature_col: np.ndarray) -> np.ndarray:
        ...
        if mapping_info['type'] == 'onehot':
            ...
            encoded = np.array([
                mapping.get(val, unknown_value) for val in feature_col
            ])
...
class DataValidator:
    ...
    def validate_training_data(
        ...
        if X.shape[0] != y.shape[0]:
            raise ValueError(f"X and y have different number of samples: {X.shape[0]} != {y.shape[0]}")
